...
/**
 * INTERNAL CHAT NEWEST HOTFIX V1.2
 *
 ...
 * php tools/apply_internal_chat_newest_hotfix_v1_2.php
 */

$root = dirname(__DIR__);
$stamp = date('Ymd-His');
$suffix = '.bak-internal-chat-newest-hotfix-v1_2-'.$stamp;
...
$manifestPath = $root.'/storage/app/internal_chat_newest_hotfix_v1_2_manifest.json';
...
function newestScriptChatHotfix(): string
{
    ...
    {{-- INTERNAL CHAT NEWEST PANEL V1.2 HOTFIX --}}
    @if ($conversation)
        <script>
            (() => {
                const bootChatNewestV12 = () => {
                    ...
                    if (! root || ! stack || root.dataset.newestPanelV12 === '1') {
                        return;
                    }

                    root.dataset.newestPanelV12 = '1';
                ...
                if (document.readyState === 'loading') {
                    document.addEventListener('DOMContentLoaded', bootChatNewestV12, { once: true });
                } else {
                    bootChatNewestV12();
                }
            })();
...
echo "INTERNAL CHAT NEWEST HOTFIX V1.2\n";
echo "=================================\n\n";
...
$manifest = [
    'version' => 'internal-chat-newest-hotfix-v1_2',
...
try {
    ...
    echo "php tools/check_internal_chat_newest_hotfix_v1_2.php\n";
} catch (Throwable $e) {
