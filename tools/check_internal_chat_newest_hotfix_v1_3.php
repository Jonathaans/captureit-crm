...
echo "CHECK INTERNAL CHAT NEWEST HOTFIX V1.3\n";
echo "=======================================\n\n";
...
checkChatHotfix(
    str_contains($chat, 'INTERNAL CHAT NEWEST PANEL V1.3 HOTFIX')
        && substr_count($chat, 'INTERNAL CHAT NEWEST PANEL V1.3 HOTFIX') === 1
        && str_contains($chat, 'window.crmChatGoNewest')
        && str_contains($chat, "document.addEventListener('submit'")
        && str_contains($chat, 'guardNewest(12000)')
        && str_contains($chat, 'scrollCandidates')
        && str_contains($chat, 'window.crmChatNewestDiagnostics')
        && str_contains($chat, 'new MutationObserver'),
    'Open/send memakai active-scroller newest controller V1.3'
);

checkChatHotfix(
    ! preg_match(
        '~\{\{--\s*INTERNAL CHAT NEWEST PANEL (?:V1|V1\.1 HOTFIX|V1\.2 HOTFIX)\s*--\}\}~',
        $chat
    ),
    'Script newest V1 sampai V1.2 lama sudah diganti'
);
